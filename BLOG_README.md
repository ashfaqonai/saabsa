...
Posts scheduled for future release live in `_posts/pending/`. GitHub Actions publishes **one post per day**:

1. **Schedule:** `publish-post.yml` runs daily at 16:20 UTC (10:20 AM CST).
2. **Publish:** `scripts/publish-pending.js` moves the oldest pending JSON to `_posts/YYYY-MM-DD_slug.json`.
3. **Deploy:** The workflow triggers `static.yml`, which runs `scripts/build-blog.js` to regenerate HTML, `posts.json`, and `sitemap.xml`.
4. **SEO:** IndexNow and optional Google Indexing API notify search engines.

### Generate the 90-day queue

Product-focused posts (Patientree AI, DataXPipe, Lease Exit, Saabsa services):

```bash
node scripts/generate-pending-posts.js
```

Use `--force` to regenerate all pending files.
...
├── pending/
│   ├── README.md
│   ├── 01-zero-cost-ai-clinic-launch-for-small-practices.json
│   ├── 02-self-scheduling-ux-fewer-abandons-more-visits.json
│   └── ... (90 files total, numbered for publish order)
├── posts.json              # Generated by build-blog.js
└── YYYY-MM-DD_slug.json    # Published posts
```

Publishing logic formerly lived in `saabsa-auto-publisher-repo`; it is now consolidated in this repository.
